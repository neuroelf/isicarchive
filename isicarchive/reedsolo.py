...
def t_code():
    c = [[
        ...
        int(r&16>0), int(r&8>0), int(r&4>0), int(r&2>0), int(r&1>0)]
        for r in range(1024)]
    for r in range(1023,-1,-1):
        cs = sum(c[r])
        if cs < 4 or cs > 6:
            c.pop(r)
        elif cs >= 5 and sa_in_la([0,0,0], c[r]):
            c.pop(r)
        elif cs <= 5 and sa_in_la([1,1,1], c[r]):
            c.pop(r)
        elif sa_in_la([0,0,0,0], c[r]):
            c.pop(r)
        elif sa_in_la([1,1,1,1], c[r]):
            c.pop(r)
        elif [0,0,0] == c[r][:3]:
            c.pop(r)
        elif [1,1,1] == c[r][:3]:
            ...
            c.pop(r)
        elif sa_in_la([0,0,1,1,0,0,1,1], c[r]):
            c.pop(r)
        elif sa_in_la([1,1,0,0,1,1,0,0], c[r]):
            c.pop(r)
    ci = dict()
    for r in range(len(c)):
        cs = sum([v * (2 ** (9-k)) for (k,v) in enumerate(c[r])])
        ci[cs] = r
    return (c, ci)
(bit_field, bit_decode) = t_code()
...
class RSCodec(object):
    ...
    def decode_bit_field(self, data, chunk_size:int = None):
        ...
        for (i,e) in enumerate(data):
            try:
                enc[i] = bit_decode[sum([v * (2 ** (9-k)) for (k,v) in enumerate(e)])]
            except:
                pass
        try:
    ...
    def value_to_bits(self, value):
        try:
            return bit_field[value]
        except:
            raise ValueError('Invalid value for bits encoding.')
